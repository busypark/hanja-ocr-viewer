document.addEventListener("DOMContentLoaded", () => {
  ...
  function redrawCanvas() {
    ctx.clearRect(0, 0, ocrCanvas.width, ocrCanvas.height);
    for (const s of strokes) drawLine(s);
  }
    ...
    for (let i = 0; i < strokes.length; i++)
      for (let j = 0; j < strokes[i].length - 1; j++)
        if (distanceToSegment(pos, strokes[i][j], strokes[i][j + 1]) <= 10) {
          strokes.splice(i, 1); redrawCanvas(); runOCR(strokes); return;
        }
  }
  ...
  function compressStrokes(raw) {
    return raw.map(s => ({ s: s.length, xy: s.map(p => [Math.floor(p.x*(283/399)), Math.floor(p.y*(283/399))]) }));
  }
  function buildInputStr(info) {
  ...
  async function runOCR(raw) {
    ...
    if (!raw || raw.length === 0) { cells.forEach(c => c.innerHTML = ""); return; }
    const inputStr = buildInputStr(compressStrokes(raw));
    try {
      const res = await fetch("/api/ocr", { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ inputStr }) });
  ...
  ocrResultTable.querySelectorAll("td").forEach(cell => {
    cell.addEventListener("click", () => {
      if (cell.innerHTML.trim()) {
        const d = cell.querySelectorAll("div");
        if (d.length > 0) searchText.value += d[0].textContent.trim();
      }
    });
  });
  ...
  document.getElementById("btnExtractMP").addEventListener("click", async () => {
    ...
    try {
      ...
      const d = result["data"];
      TA_1.value = d["MP"] + "\n\n" + JSON.parse(d["learningMoreList"]).join("\n\n");
