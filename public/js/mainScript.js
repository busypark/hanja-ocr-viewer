document.addEventListener("DOMContentLoaded", () => {
  ...
  function switchMode(mode) { currentMode = mode; btnPen.disabled = (mode==="pen"); btnErase.disabled = (mode==="erase"); }
  ...
  function getPos(e) { const r = ocrCanvas.getBoundingClientRect(); return { x: e.clientX - r.left, y: e.clientY - r.top }; }
  ...
  ocrCanvas.addEventListener("mousedown", e => { const p = getPos(e); if (currentMode==="pen") { isDrawing=true; currentStroke=[p]; } else eraseAt(p); });
  ocrCanvas.addEventListener("mousemove", e => { const p=getPos(e); if(currentMode==="pen"&&isDrawing){currentStroke.push(p);drawLine(currentStroke.slice(-2));}else if(currentMode==="erase")eraseAt(p); });
  function endStroke() { if(!isDrawing)return; if(currentStroke.length>1){strokes.push(currentStroke);runOCR(strokes);} isDrawing=false;currentStroke=[]; }
  ...
  async function runOCR(raw) {
    ...
    try {
      ...
      for(let i=0;i<Math.min(items.length,20);i++){const[char,desc]=items[i].split(",");if(!desc)continue;cells[idx++].innerHTML=`<div style="font-size:40px;color:gold;text-align:center">${char}</div><div style="font-size:18px;text-align:center">${desc}</div>`;}
    } catch(err){console.error("OCR 실패:",err);}
  }

  ocrResultTable.querySelectorAll("td").forEach(cell=>{cell.addEventListener("click",()=>{if(cell.innerHTML.trim()){const d=cell.querySelectorAll("div");if(d.length)searchText.value+=d[0].textContent.trim();}});});
  ...
  document.getElementById("btnSearchN").addEventListener("click",()=>{const q=searchText.value.trim();if(q)window.open("https://hanja.dict.naver.com/#/search?query="+encodeURIComponent(q),"_blank");});
  document.getElementById("btnExtractMP").addEventListener("click",async()=>{
    ...
    try{const res=await fetch("/api/ExtractMP",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:sText})});const result=await res.json();const d=result["data"];TA_1.value=d["MP"];TA_2.value=JSON.parse(d["learningMoreList"]).join("\n\n");}
    catch(err){alert("네트워크 오류: "+err.message);}
  });
  document.getElementById("btnExtractWords").addEventListener("click",async()=>{
    const sText=searchText.value.trim(); if(!sText){alert("검색어가 비어 있습니다.");return;}
    try{const res=await fetch("/api/ExtractWords",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:sText})});const result=await res.json();const d=result["data"];TA_1.value=d["Words"];TA_2.value=d["Idioms"];}
    catch(err){alert("네트워크 오류: "+err.message);}
  });
  ...
  // Zdic button - 서버 요청 확인용 (char 탭 미구현)
  document.getElementById("btnSearchZ").addEventListener("click", async () => {
    ...
    try {
      ...
      console.log('Zdic SVG result:', data);
    } catch (err) {
      alert(err.message);
    }
  });
